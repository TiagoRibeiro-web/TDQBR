...
st.set_page_config(page_title="Dashboard QBR TD SYNNEX", layout="wide", initial_sidebar_state="expanded")

# ========== CARREGANDO CREDENCIAIS DO st.secrets ==========
try:
    ...
    st.error("Erro ao carregar credenciais do st.secrets. Certifique-se de que o arquivo .streamlit/secrets.toml está configurado corretamente.")
    st.stop()
...
CURRENT_CAMPANHAS_DATA = {
    'Q1': {
        'solicitadas': 6, 'veiculadas': 4, 'taxa_conversao': 7.26, 'leads_gerados': 62, 'conversao': 83, 'branding': 17,
        'top_campanhas': '🏆 Fortinet Roadshow (56 empresas)<br>🏆 NVIDIA IA (6 empresas)'
    },
    'Q2': {
        'solicitadas': 9, 'veiculadas': 6, 'taxa_conversao': None, 'leads_gerados': None, 'conversao': None, 'branding': None,
        'top_campanhas': '🏆 IBM IA (55 empresas)<br>🏆 Microsoft Roadshow (40 empresas)<br>🏆 Fortinet Recrutamento (25 empresas)'
    },
    'Q3': {
        'solicitadas': 7, 'veiculadas': 6, 'taxa_conversao': None, 'leads_gerados': None, 'conversao': None, 'branding': None,
        'top_campanhas': '🏆 Cloud On the Go (383 empresas)<br>🏆 Fortinet Roadshow (46 empresas)<br>🏆 NVIDIA IA (10 empresas)'
    },
    'Q4': {
        'solicitadas': 6, 'veiculadas': 3, 'taxa_conversao': None, 'leads_gerados': 70, 'conversao': None, 'branding': None,
...
<style>
    ...
    * { font-family: 'Inter', sans-serif; }
    ...
    .premium-header {
        ...
        backdrop-filter: blur(10px); padding: 48px 56px 40px 56px; margin-bottom: 32px; position: relative; overflow: hidden; border-bottom: 1px solid rgba(255,255,255,0.1);
    }
    .premium-title { font-size: 52px; font-weight: 800; background: linear-gradient(135deg, #FFFFFF, #E0E0E0); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; margin: 0; letter-spacing: -1px; }
    ...
    .comparison-premium { border-radius: 16px; padding: 12px; text-align: left; margin-top: 12px; }
    .comparison-header { font-size: 10px; margin-bottom: 10px; text-transform: uppercase; letter-spacing: 1px; }
    .comparison-row { display: grid; grid-template-columns: auto 1fr auto; align-items: center; gap: 8px; margin-bottom: 8px; font-size: 11px; }
    .comp-badge { display: inline-flex; align-items: center; gap: 3px; padding: 2px 8px; border-radius: 20px; font-size: 10px; font-weight: 700; white-space: nowrap; }
    ...
    div.stButton > button { padding: 10px 28px; border-radius: 40px; background: rgba(0,0,0,0.2); backdrop-filter: blur(5px); font-size: 14px; font-weight: 600; transition: all 0.3s ease; }
...
<style>
    ...
    .glass-card {{ background: {COLORS['card_bg']}; backdrop-filter: blur(12px); border-radius: 24px; padding: 24px; border: 1px solid {COLORS['border']}; transition: all 0.4s ease; margin-bottom: 24px; height: 100%; }}
    .glass-card:hover {{ transform: translateY(-6px); border-color: {COLORS['primary']}; box-shadow: 0 20px 40px rgba(0,0,0,0.3); }}
    .kpi-number {{ font-size: 48px; font-weight: 800; line-height: 1; background: {COLORS['kpi_number_bg']}; -webkit-background-clip: text; background-clip: text; color: {COLORS['kpi_number_color']}; }}
    ...
    .premium-quarter {{ font-size: 24px; font-weight: 600; color: {COLORS['secondary']}; margin-top: 16px; display: inline-block; background: rgba(0,0,0,0.3); padding: 6px 20px; border-radius: 40px; }}
    .comparison-premium {{ background: rgba(0,0,0,0.12); border: 1px solid {COLORS['border']}; }}
    ...
    .blog-premium {{ background: {COLORS['card_bg']}; backdrop-filter: blur(12px); border-radius: 24px; padding: 28px; border: 1px solid {COLORS['border']}; transition: all 0.4s ease; height: 100%; position: relative; overflow: hidden; margin-bottom: 24px; }}
    .blog-premium::before {{ content: ''; position: absolute; top: 0; left: 0; right: 0; height: 3px; background: linear-gradient(90deg, {COLORS['primary']}, {COLORS['secondary']}, {COLORS['accent']}); }}
    .blog-header-premium {{ display: flex; align-items: center; gap: 12px; margin-bottom: 24px; padding-bottom: 16px; border-bottom: 1px solid {COLORS['border']}; }}
    .blog-title-premium {{ font-size: 20px; font-weight: 700; color: {COLORS['text']}; }}
    .stDataFrame thead th {{ background: linear-gradient(135deg, rgba(0,102,204,0.3), rgba(123,44,191,0.2)) !important; color: {COLORS['text']} !important; font-weight: 600 !important; }}
    .stDataFrame tbody td {{ background: rgba(15,43,61,0.3) !important; color: {COLORS['text']} !important; border-bottom: 1px solid {COLORS['border']} !important; }}
    div.stButton > button {{ border: 1px solid {COLORS['border']}; color: {COLORS['text']}; }}
</style>
"""
...
def clean_percentage(value):
    ...
    if '%' not in value_str: return value_str
...
def clean_value(val):
    ...
    return '—' if val_str == '' else val_str
...
def calc_variacao(atual, anterior):
    if atual in (None, '—') or anterior in (None, '—'): return None
    try:
...
def get_current_email_value(q, metric):
    if q and q in CURRENT_EMAIL_DATA:
        ...
        if val is not None: return f"{val}%"
    return '—'
...
def get_current_newsletter_value(q, metric):
    if q and q in CURRENT_NEWSLETTER_DATA:
        ...
        if val is not None: return f"{val}%" if metric in ['abertura', 'cliques'] else f"{val:,}".replace(',', '.')
    return '—'
...
def get_campanha_hardcoded(q, metric):
    if q in CURRENT_CAMPANHAS_DATA and metric in CURRENT_CAMPANHAS_DATA[q]:
        ...
        if val is not None:
            if metric in ['taxa_conversao', 'conversao', 'branding']: return f"{val}%"
            return str(val)
    return None
...
def render_kpi_premium(valor, label, icone, variacao=None, trimestre_ref=None, tooltip=None):
    ...
    if variacao is not None and trimestre_ref:
        if variacao > 0: badge = f'<div class="kpi-variation kpi-up">↑ {variacao:.1f}% <span style="font-size:9px;">vs {trimestre_ref}</span></div>'
        elif variacao < 0: badge = f'<div class="kpi-variation kpi-down">↓ {abs(variacao):.1f}% <span style="font-size:9px;">vs {trimestre_ref}</span></div>'
        else: badge = f'<div class="kpi-variation kpi-neutral">→ 0% <span style="font-size:9px;">vs {trimestre_ref}</span></div>'
    st.html(f"""
...
def _comp_badge(variacao):
    if variacao is None: return '<span class="comp-badge badge-flat">—</span>'
...
def render_metric_card(metric_name, icon, current_value, prev_fy25_value, prev_fy25_name, same_fy24_value, same_fy24_name, is_percentage=True, tooltip=None):
    ...
    <div class="glass-card" style="text-align:center;">
        ...
        <div class="comparison-premium">
            ...
            <div class="comparison-row"><span class="comp-label">{prev_fy25_name}</span><span class="comp-value">{prev_str}{suffix}</span>{_comp_badge(var_prev)}</div>
            <div class="comparison-row"><span class="comp-label">{same_fy24_name}</span><span class="comp-value">{same_str}{suffix}</span>{_comp_badge(var_same)}</div>
        </div>
    </div>
    """)

def render_blog_item(label, value, prev_fy25_value, prev_fy25_name, same_fy24_value, same_fy24_name, is_percentage=False, icon="📊"):
    ...
    <div class="blog-item-premium">
        ...
        <div class="blog-compare-premium"><span>vs {prev_fy25_name}: {prev_str}{suffix}</span>{badge(var_prev)}</div>
        <div class="blog-compare-premium" style="margin-top:4px;"><span>vs {same_fy24_name}: {same_str}{suffix}</span>{badge(var_same)}</div>
    </div>
    """)

def render_horizontal_bars(df, title):
    if df is None or df.empty: return
    vertical_colors = {'Segurança':'#D32F2F','Networking':'#0066CC','Cloud':'#3399FF','Data & AI':'#7B2CBF','Data Center':'#FF6B35','Institucional':'#2E7D32'}
    df = df.copy()
    ...
    for _, row in df.iterrows():
        ...
        <div class="bar-premium">
            <div class="bar-label-premium"><span>{row['vertical']}</span><span style="color:{color};">{pct:.1f}%</span></div>
            <div class="bar-track-premium"><div class="bar-fill-premium" style="width:{pct}%;background:{color};"></div></div>
        </div>
        """, unsafe_allow_html=True)
...
def render_table(df, title=None):
    ...
    if title: st.markdown(f'<div style="font-size:16px;font-weight:600;color:{COLORS["text"]};margin-bottom:16px;">{title}</div>', unsafe_allow_html=True)
...
def render_comparative_charts(quarterly_data):
    ...
    for key, label, color in [('pecas','Peças',COLORS['primary']),('solic','Solicitações',COLORS['secondary']),('camp','Campanhas',COLORS['success'])]:
        ...
        fig = go.Figure()
        fig.add_trace(go.Bar(x=df_c['trimestre'], y=df_c['valor'], name=label, marker=dict(color=color), text=df_c['valor'], textposition='outside', textfont=dict(color=COLORS['text'])))
        fig.add_trace(go.Scatter(x=df_c['trimestre'], y=df_c['valor'], mode='lines+markers', name='Tendência', line=dict(color=COLORS['secondary'] if key!='solic' else COLORS['primary'], width=3), marker=dict(size=10)))
        fig.update_layout(title=f'{label} por Trimestre', title_font_color=COLORS['text'], paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', xaxis=dict(tickfont_color=COLORS['text'], gridcolor=COLORS['border']), yaxis=dict(tickfont_color=COLORS['text'], gridcolor=COLORS['border']), legend=dict(font_color=COLORS['text']), height=400)
...
def load_all_data():
    try:
        fabricantes = remove_duplicates(query_db(f"SELECT * FROM {SCHEMA}.fabricantes_campanhas ORDER BY trimestre"), ['trimestre', 'vertical'])
        return {
...
if 'view' not in st.session_state: st.session_state.view = 'Q1'
...
quarterly_data = {t: {'pecas': get_pecas_value(data['overview'], t), 'solic': get_solicitacoes_value(data['overview'], t), 'camp': get_campanhas_value(data['overview'], t)} for t in ['Q1', 'Q2', 'Q3', 'Q4']}
...
if q == 'FY25':
    ...
    pecas_tot, solic_tot, camp_tot = sum(quarterly_data[t]['pecas'] for t in ['Q1','Q2','Q3','Q4']), sum(quarterly_data[t]['solic'] for t in ['Q1','Q2','Q3','Q4']), sum(quarterly_data[t]['camp'] for t in ['Q1','Q2','Q3','Q4'])
    ...
    for t in ['Q1','Q2','Q3','Q4']:
        for _, row in get_vertical_distribution(t).iterrows(): vert_vals.setdefault(row['vertical'], []).append(row['percentual'])
    ...
    top_camp = {'Q1':'Fortinet Roadshow (56 empresas) | NVIDIA IA (6 empresas)', 'Q2':'IBM IA (55) | Microsoft Roadshow (40) | Fortinet Recrutamento (25)', 'Q3':'Cloud On the Go (383) | Fortinet Roadshow (46) | NVIDIA IA (10)', 'Q4':'Campanha Recrutamento Fortinet (70 empresas)'}
    c1a, c2a, c3a, c4a = st.columns(4)
    for col, (t, camp) in zip([c1a,c2a,c3a,c4a], top_camp.items()):
        with col: st.markdown(f'<div class="glass-card"><div style="font-size:14px;font-weight:600;color:{COLORS["primary"]};margin-bottom:12px;">{t} FY25</div><div style="font-size:12px;line-height:1.5;color:{COLORS["text"]};">{camp}</div></div>', unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        if st.button("📥 Baixar FY25", key="btn_dl_fy25"):
            if dl_fmt == "CSV": st.download_button("✅ Baixar CSV", df_dl.to_csv(index=False), "qbr_fy25.csv", "text/csv")
            else:
                buf = io.BytesIO()
    ...
    pecas_val, solic_val, camp_val = quarterly_data[q]['pecas'], quarterly_data[q]['solic'], quarterly_data[q]['camp']
    if prev_fy25_q:
        pecas_var, solic_var, camp_var = calc_variacao(pecas_val, quarterly_data[prev_fy25_q]['pecas']), calc_variacao(solic_val, quarterly_data[prev_fy25_q]['solic']), calc_variacao(camp_val, quarterly_data[prev_fy25_q]['camp'])
        kpi_ref = prev_fy25_name
    else: pecas_var = solic_var = camp_var = kpi_ref = None
    tooltips = {'entrega': 'Percentual de e-mails entregues com sucesso', 'abertura': 'Percentual de e-mails abertos pelos destinatários', 'cliques': 'Percentual de cliques nos e-mails enviados'}
    ...
    with cc1:
        ...
        if camp_solic == '—': camp_solic = get_campanha_valor(data['campanhas'], 'Campanhas Solicitadas', q)
        if camp_solic == '—': camp_solic = get_campanha_hardcoded(q, 'solicitadas') or '—'
        camp_veic = get_campanha_valor(data['campanhas'], 'Veiculadas', q)
        if camp_veic == '—': camp_veic = get_campanha_valor(data['campanhas'], 'Campanhas Veiculadas', q)
        if camp_veic == '—': camp_veic = get_campanha_hardcoded(q, 'veiculadas') or '—'
        conv_invest = get_campanha_valor(data['campanhas'], 'Taxa de Conversão (com investimento)', q)
        if conv_invest == '—': conv_invest = get_campanha_valor(data['campanhas'], 'Taxa de Conversão (c/ invest.)', q)
        if conv_invest == '—': conv_invest = get_campanha_hardcoded(q, 'taxa_conversao') or '—'
        leads = get_campanha_valor(data['campanhas'], 'MQLs Gerados (empresas)', q)
        if leads == '—': leads = get_campanha_valor(data['campanhas'], 'Leads / Empresas Gerados', q)
        if leads == '—': leads = get_campanha_hardcoded(q, 'leads_gerados') or '—'
        st.html(f"""<div class="glass-card"><div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:20px;"><div style="text-align:center;"><div style="font-size:11px;color:{COLORS['text_muted']};">Solicitadas</div><div style="font-size:36px;font-weight:800;color:{COLORS['text']};">{camp_solic}</div></div><div style="text-align:center;"><div style="font-size:11px;color:{COLORS['text_muted']};">Veiculadas</div><div style="font-size:36px;font-weight:800;color:{COLORS['text']};">{camp_veic}</div></div></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;"><div style="text-align:center;"><div style="font-size:11px;color:{COLORS['text_muted']};">Taxa Conversão</div><div style="font-size:28px;font-weight:800;color:#6FBF6F;">{conv_invest}</div></div><div style="text-align:center;"><div style="font-size:11px;color:{COLORS['text_muted']};">Leads Gerados</div><div style="font-size:28px;font-weight:800;color:{COLORS['text']};">{leads}</div></div></div></div>""")
    with cc2:
        top_text = get_campanha_hardcoded(q, 'top_campanhas') or {'Q1':'🏆 Fortinet Roadshow (56 empresas)<br>🏆 NVIDIA IA (6 empresas)', 'Q2':'🏆 IBM IA (55 empresas)<br>🏆 Microsoft Roadshow (40 empresas)<br>🏆 Fortinet Recrutamento (25 empresas)', 'Q3':'🏆 Cloud On the Go (383 empresas)<br>🏆 Fortinet Roadshow (46 empresas)<br>🏆 NVIDIA IA (10 empresas)', 'Q4':'🏆 Campanha Recrutamento Fortinet (70 empresas)'}.get(q, '—')
        obj_conv = get_campanha_valor(data['campanhas'], 'Conversão', q)
        if obj_conv == '—': obj_conv = get_campanha_valor(data['campanhas'], 'Objetivo Conversão', q)
        if obj_conv == '—': obj_conv = get_campanha_hardcoded(q, 'conversao') or '—'
        obj_brand = get_campanha_valor(data['campanhas'], 'Branding', q)
        if obj_brand == '—': obj_brand = get_campanha_valor(data['campanhas'], 'Objetivo Branding', q)
        if obj_brand == '—': obj_brand = get_campanha_hardcoded(q, 'branding') or '—'
        st.html(f"""<div class="glass-card"><div style="font-size:18px;font-weight:700;color:{COLORS['text']};margin-bottom:16px;">🏆 Top Campanhas</div><div style="font-size:13px;line-height:1.6;color:{COLORS['text_muted']};margin-bottom:20px;">{top_text}</div><div style="font-size:14px;font-weight:600;color:{COLORS['text']};margin:16px 0 12px;">📊 Distribuição por Objetivo</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;"><div style="background:rgba(0,0,0,0.05);border-radius:16px;padding:12px;text-align:center;"><div style="font-size:11px;color:{COLORS['text_muted']};">Conversão</div><div style="font-size:28px;font-weight:800;color:#6FBF6F;">{obj_conv}</div></div><div style="background:rgba(0,0,0,0.05);border-radius:16px;padding:12px;text-align:center;"><div style="font-size:11px;color:{COLORS['text_muted']};">Branding</div><div style="font-size:28px;font-weight:800;color:#FF8C5A;">{obj_brand}</div></div></div></div>""")
    ...
    if not df_fab.empty:
        ...
        render_table(pd.DataFrame({'Vertical': [clean_value(r['vertical']) for _, r in df_fab.iterrows()], 'Maior Solicitante': [clean_value(r['maior_solicitante']) for _, r in df_fab.iterrows()], 'Menor Solicitante': [clean_value(r['menor_solicitante']) for _, r in df_fab.iterrows()], 'Top 3 Peças': [clean_value(r['top_3_pecas']) for _, r in df_fab.iterrows()], 'Campanha Ativa': [clean_value(r['campanha_ativa']) if r['campanha_ativa'] not in ['—',None] else '—' for _, r in df_fab.iterrows()]}), "🏭 Fabricantes por Vertical")
    ...
    entrega_atual, abertura_atual, cliques_atual, optout_atual = get_current_email_value(q, 'entrega'), get_current_email_value(q, 'abertura'), get_current_email_value(q, 'cliques'), get_current_email_value(q, 'optout')
    prev_entrega, prev_abertura, prev_cliques, prev_optout = (get_current_email_value(prev_fy25_q, 'entrega') if prev_fy25_q else '—'), (get_current_email_value(prev_fy25_q, 'abertura') if prev_fy25_q else '—'), (get_current_email_value(prev_fy25_q, 'cliques') if prev_fy25_q else '—'), (get_current_email_value(prev_fy25_q, 'optout') if prev_fy25_q else '—')
    same_fy24_email = HISTORICAL_DATA['email'].get(f'{q}FY24', {})
    same_entrega, same_abertura, same_cliques, same_optout = same_fy24_email.get('entrega', '—'), same_fy24_email.get('abertura', '—'), same_fy24_email.get('cliques', '—'), same_fy24_email.get('optout', '—')
    e1, e2, e3, e4 = st.columns(4)
    with e1: render_metric_card("Entregas", "✅", entrega_atual, prev_entrega, prev_fy25_name, same_entrega, same_fy24_name, tooltip=tooltips['entrega'])
    with e2: render_metric_card("Aberturas", "👁️", abertura_atual, prev_abertura, prev_fy25_name, same_abertura, same_fy24_name, tooltip=tooltips['abertura'])
    with e3: render_metric_card("Cliques", "🖱️", cliques_atual, prev_cliques, prev_fy25_name, same_cliques, same_fy24_name, tooltip=tooltips['cliques'])
    with e4: render_metric_card("Opt-Out", "🚫", optout_atual, prev_optout, prev_fy25_name, same_optout, same_fy24_name)
    df_ev = data['email_vertical'][data['email_vertical']['trimestre'] == q]
    if not df_ev.empty: render_table(df_ev[['vertical_tipo','percent_envio','entrega','abertura','clique','opt_out']], "📊 Métricas de E-mail por Vertical")
    ...
    seg_atual, eng_atual, cli_atual = _get_rede(df_rg, 'Novos Seguidores'), _get_rede(df_rg, 'Total de Engajamentos'), _get_rede(df_rg, 'Total de Cliques')
    if prev_fy25_q:
        df_rg_prev = data['redes_geral'][data['redes_geral']['trimestre'] == prev_fy25_q]
        prev_seg, prev_eng, prev_cli = _get_rede(df_rg_prev, 'Novos Seguidores'), _get_rede(df_rg_prev, 'Total de Engajamentos'), _get_rede(df_rg_prev, 'Total de Cliques')
    else: prev_seg = prev_eng = prev_cli = '—'
    same_fy24_redes = HISTORICAL_DATA['redes'].get(f'{q}FY24', {})
    same_seg, same_eng, same_cli = same_fy24_redes.get('seguidores', '—'), same_fy24_redes.get('engajamentos','—'), same_fy24_redes.get('cliques', '—')
    ...
    if not df_rr.empty: render_table(df_rr[['rede','seguidores','media_seguidores','engajamentos','cliques']], "📊 Métricas por Rede Social")
    ...
    with bl_col:
        st.html(f"""<div class="blog-premium"><div class="blog-header-premium"><div class="blog-icon-premium">📖</div><div><div class="blog-title-premium">Blog</div><div class="blog-sub-premium">Conteúdos e performance</div></div></div></div>""")
        for metric, label, icon, same_fy24_key in [('visitas', 'Visitas', '👁️', 'visitas'), ('usuarios', 'Usuários', '👥', 'usuarios'), ('blogposts', 'Blogposts Publicados', '📝', None), ('tempo_medio','Tempo Médio na Página','⏱️', None)]:
            render_blog_item(label=label, value=get_current_blog_value(q, metric), prev_fy25_value=get_current_blog_value(prev_fy25_q, metric) if prev_fy25_q else '—', prev_fy25_name=prev_fy25_name, same_fy24_value=same_fy24_blog.get(same_fy24_key, '—') if same_fy24_key else '—', same_fy24_name=same_fy24_name if same_fy24_key else '—', icon=icon)
    with nw_col:
        st.html(f"""<div class="blog-premium"><div class="blog-header-premium"><div class="blog-icon-premium">📬</div><div><div class="blog-title-premium">Newsletter</div><div class="section-sub">Comunicação direta</div></div></div></div>""")
        for metric, label, icon, same_fy24_key, is_pct in [('empresas', 'Empresas', '🏢', None, False), ('envios', 'Envios', '📨', None, False), ('abertura', 'Taxa de Abertura','📊', 'abertura_news', True), ('cliques', 'Taxa de Cliques', '🖱️', 'cliques_news', True)]:
            render_blog_item(label=label, value=get_current_newsletter_value(q, metric), prev_fy25_value=get_current_newsletter_value(prev_fy25_q, metric) if prev_fy25_q else '—', prev_fy25_name=prev_fy25_name, same_fy24_value=same_fy24_blog.get(same_fy24_key, '—') if same_fy24_key else '—', same_fy24_name=same_fy24_name if same_fy24_key else '—', is_percentage=is_pct, icon=icon)
    ...
    with ap1: st.markdown(f"""<div class="glass-card"><div style="font-size:18px;font-weight:700;color:#6FBF6F;margin-bottom:16px;">✅ QUE BOM</div><div style="font-size:12px;line-height:1.6;color:{COLORS['text_muted']};">• <strong>PowerUP</strong>: Plataforma de inscrição, estratégias online e metas diárias<br>• <strong>Case Recrutamento Fortinet</strong>: 80 Leads — virou referência<br>• <strong>Segmentação Microsoft</strong>: Timing nos e-mails aumentou quórum<br>• <strong>Campanha BU Networking</strong>: Insights valiosos para futuras BUs<br>• <strong>Melhora nas cadências</strong>: Contato mais frequente para alinhar jobs</div></div>""", unsafe_allow_html=True)
    with ap2: st.markdown(f"""<div class="glass-card"><div style="font-size:18px;font-weight:700;color:#FF8C5A;margin-bottom:16px;">⚠️ QUE PENA</div><div style="font-size:12px;line-height:1.6;color:{COLORS['text_muted']};">• <strong>Qualidade das bases</strong>: Necessidade de segmentar melhor os parceiros<br>• <strong>Análise constante de resultados</strong>: Todos os jobs precisam ser acompanhados</div></div>""", unsafe_allow_html=True)
    with ap3: st.markdown(f"""<div class="glass-card"><div style="font-size:18px;font-weight:700;color:#3399FF;margin-bottom:16px;">💡 QUE TAL — 2026</div><div style="font-size:12px;line-height:1.6;color:{COLORS['text_muted']};">• <strong>Intercâmbio de treinamentos</strong>: Calendário ao longo do ano (ID ↔ TDS)<br>• <strong>Alinhamentos estratégicos recorrentes</strong>: Incluir agência no planejamento<br>• <strong>Metodologia EDGE</strong>: Otimização do formulário de briefing<br>• <strong>Retomar alinhamentos institucionais</strong>: Contato próximo com assessoria</div></div>""", unsafe_allow_html=True)
    with st.sidebar:
        st.markdown("---")
        st.markdown("### 📥 Exportar Dados")
        dl_fmt = st.selectbox("Formato", ["CSV", "Excel"], key="dl_q")
        if st.button(f"📥 Baixar {q}", key="btn_dl_q"):
            rows = [{'Categoria':'Peças Produzidas','Valor':pecas_val}, {'Categoria':'Solicitações','Valor':solic_val}, {'Categoria':'Campanhas','Valor':camp_val}, {'Categoria':'Campanhas Solicitadas','Valor':camp_solic}, {'Categoria':'Campanhas Veiculadas','Valor':camp_veic}, {'Categoria':'Taxa Conversão','Valor':conv_invest}, {'Categoria':'Leads Gerados','Valor':leads}, {'Categoria':'Taxa Entrega E-mail','Valor':entrega_atual}, {'Categoria':'Taxa Abertura E-mail','Valor':abertura_atual}, {'Categoria':'Taxa Cliques E-mail','Valor':cliques_atual}, {'Categoria':'Novos Seguidores','Valor':seg_atual}, {'Categoria':'Total Engajamentos','Valor':eng_atual}, {'Categoria':'Total Cliques','Valor':cli_atual}]
            df_dl = pd.DataFrame(rows)
            if dl_fmt == "CSV": st.download_button("✅ Baixar CSV", df_dl.to_csv(index=False), f"qbr_{q}.csv", "text/csv")
            else:
                buf = io.BytesIO()
...
st.markdown(f"""<div class="footer-premium">⚡ QBR TD SYNNEX — Powered by Ideatore · Dados atualizados em {datetime.now().strftime('%d/%m/%Y %H:%M')}</div>""", unsafe_allow_html=True)
st.markdown('</div>', unsafe_allow_html=True)
